  ...
  return <ScrollView contentContainerStyle={s.content}>
    ...
    {loading?<ActivityIndicator color={colors.accent}/>:<>
      <View style={s.quickStats}>
        ...
        <ProgressStat color={colors.success} label={recentRows.length?`${average}/100 average feeling · past 7 days`:'Average feeling will appear after your first check-in'}/>
      ...
      <ProgressDropdown label="REPORTED FEELING" onPress={()=>setFeelingsOpen(value=>!value)} open={feelingsOpen}>
        {rows.length===0?<Text style={s.dropdownEmpty}>Complete your first morning check-in and it will appear here.</Text>:rows.map(row=><View key={row.checkin_date} style={s.feelingHistoryRow}><Text style={s.rowDate}>{formatProgressDate(row.checkin_date)}</Text><Text style={s.rowValue}>{row.feeling}/100</Text></View>)}
      </ProgressDropdown>
    </>}
