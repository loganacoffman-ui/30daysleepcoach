...
const all = JSON.parse(await readFile(resolve(root, value('--dataset', 'cases.json')), 'utf8'));
...
const manifest = { split, cases: cases.map(c => c.id), dataset_sha256: hash(JSON.stringify(all)),
  ...
  context_builder_sha256: hash(await readFile(resolve(root, '../../supabase/functions/_shared/dailyCoachingContext.ts'), 'utf8')),
  ...
  for (const [variant, system] of Object.entries(prompts)) {
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({ model: manifest.model, max_tokens: manifest.max_output_tokens, system,
        messages: [{ role: 'user', content: buildDailyCoachingMessage(example.context, example.context.current_daily_experiment ?? null) }] }),
    });
    ...
    await appendFile(resolve(out, 'results.jsonl'), JSON.stringify({ case_id: example.id, variant,
      ...
      grounded_pattern: groundedDailyPattern(example.context), latency_ms: Date.now() - started, usage: data.usage, model: data.model, stop_reason: data.stop_reason,
