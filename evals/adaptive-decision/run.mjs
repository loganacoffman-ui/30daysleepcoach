...
import {parseDailyDecision,DAILY_DECISION_TOOL,dailyDecisionToolChoice} from '../../supabase/functions/_shared/dailyDecision.ts';
const root=fileURLToPath(new URL('../../',import.meta.url));
const cases=JSON.parse(readFileSync(new URL('./cases.json',import.meta.url),'utf8'));
const hash=s=>createHash('sha256').update(s).digest('hex');
const payload={tool:DAILY_DECISION_TOOL,tool_choice:dailyDecisionToolChoice,system:ADAPTIVE_DAILY_SYSTEM_PROMPT,cases:cases.map(c=>({...c,message:buildDailyCoachingMessage(c.context,c.current)}))};
const manifest={tool_schema_sha256:hash(JSON.stringify(DAILY_DECISION_TOOL)),model:'claude-sonnet-4-6',max_tokens:1000,cases:cases.map(c=>c.id),prompt_sha256:hash(payload.system),dataset_sha256:hash(JSON.stringify(cases)),message_sha256:hash(JSON.stringify(payload.cases.map(c=>c.message)))};
...
Deno.serve(async req=>{
 ...
 const response=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',signal:AbortSignal.timeout(55000),headers:{'content-type':'application/json','anthropic-version':'2023-06-01','x-api-key':Deno.env.get('ANTHROPIC_API_KEY')!},body:JSON.stringify({model:'claude-sonnet-4-6',max_tokens:1000,system:data.system,tools:[data.tool],tool_choice:data.tool_choice,messages:[{role:'user',content:message}]})});
 if(!response.ok)throw Error('Provider request failed');return await response.json();};
 const first=await generate(c.message);const draft=parseDailyDecision(dailyDecisionOutput(first),c.current);
 if(!draft)return Response.json({error:'Invalid draft'},{status:502});
 const r=first;
 return Response.json({case_id:c.id,model:r.model,usage:r.usage,stop_reason:r.stop_reason,latency_ms:Date.now()-start,output:dailyDecisionOutput(r)});
});`);
...
try {
 ...
 for(const c of cases){
  ...
  const result={...raw,split:c.split,expected:c.expected,rubric:c.rubric,parsed,displayed:parsed?{...parsed,pattern:groundedDailyPattern(c.context)??parsed.pattern}:null,decision_check:!!parsed&&(c.expected==='adapt'?['simplify','replace'].includes(parsed.decision):parsed.decision===c.expected)};
...
if(results.length!==cases.length||results.some(r=>!r.decision_check))process.exitCode=1;
